---
ticket_id: CITSD-68
title: After Validation and Verification were submitted by the contractor, a notification
  email was not sent to the appropriate DERProcessing inbox.
status: Closed
created: '2022-03-04'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-68
project: citsd
tags:
- jira
- ticket
- citsd
- closed
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# CITSD-68: After Validation and Verification were submitted by the contractor, a notification email was not sent to the appropriate DERProcessing inbox.
## Overview
- **Status**: Closed
- **Created**: 2022-03-04
- **Project**: citsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-68)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*