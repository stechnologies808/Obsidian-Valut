---
ticket_id: HESD-26
title: CSV Reporting Export headers are always field names; previous to CID go-live
  it was the Display Fields Using
status: Closed
created: '2022-03-21'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-26
project: hesd
tags:
- jira
- ticket
- closed
- hesd
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# HESD-26: CSV Reporting Export headers are always field names; previous to CID go-live it was the Display Fields Using
## Overview
- **Status**: Closed
- **Created**: 2022-03-21
- **Project**: hesd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-26)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*