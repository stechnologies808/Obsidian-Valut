---
ticket_id: HESD-240
title: P-CSS-22-518533  Project address updated with Sarah Change Field Value; change
  not reflected in application
status: Closed
created: '2023-05-26'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-240
project: hesd
tags:
- jira
- ticket
- closed
- hesd
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# HESD-240: P-CSS-22-518533 Project address updated with Sarah Change Field Value; change not reflected in application
## Overview
- **Status**: Closed
- **Created**: 2023-05-26
- **Project**: hesd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-240)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*