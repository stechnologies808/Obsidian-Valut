---
# Dataview Properties
ticket_id: "CITSD-311"
title: "ALM 134 TEST - REVISION REVIEW - the "Not Listed" field for adding and removing inverter and panels indicate "YES" when "yes" was not marked during the revision."
status: "Closed"
created: "2023-04-27"
ticket_url: "https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-311"
project: "citsd"
  - jira
  - ticket
  - citsd
  - closed
updated: "2025-11-15T07:16"
last_synced: "2025-11-15T07:16"---

# CITSD-311: ALM 134 TEST - REVISION REVIEW - the "Not Listed" field for adding and removing inverter and panels indicate "YES" when "yes" was not marked during the revision.
## Overview
- **Status**: Closed
- **Created**: 2023-04-27
- **Project**: citsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-311)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*