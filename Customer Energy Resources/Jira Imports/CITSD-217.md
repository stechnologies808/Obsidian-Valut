---
# Dataview Properties
ticket_id: "CITSD-217"
title: "ALM-44 TEST UAT: CIT Submitted Application "Page not found" when attempting to Withdraw an application"
status: "Resolved"
created: "2023-03-24"
ticket_url: "https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-217"
project: "citsd"
  - jira
  - ticket
  - citsd
  - resolved
updated: "2025-11-15T07:16"
last_synced: "2025-11-15T07:16"---

# CITSD-217: ALM-44 TEST UAT: CIT Submitted Application "Page not found" when attempting to Withdraw an application
## Overview
- **Status**: Resolved
- **Created**: 2023-03-24
- **Project**: citsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-217)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*