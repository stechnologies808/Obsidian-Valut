---
# Dataview Properties
ticket_id: "HESD-422"
title: "ARD730 - "BYOD Committed Capacity" is empty for some rows in CSV extract"
status: "Resolved"
created: "2024-06-06"
ticket_url: "https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-422"
project: "hesd"
  - jira
  - ticket
  - hesd
  - resolved
updated: "2025-11-15T07:16"
last_synced: "2025-11-15T07:16"---

# HESD-422: ARD730 - "BYOD Committed Capacity" is empty for some rows in CSV extract
## Overview
- **Status**: Resolved
- **Created**: 2024-06-06
- **Project**: hesd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-422)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*