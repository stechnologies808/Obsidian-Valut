---
# Dataview Properties
ticket_id: "HESD-466"
title: "ARD795 - [AppStuck] Completeness Review: Processor receiving error message when moving application to CAR status Error Message "Error occurred during processing""
status: "Closed"
created: "2024-07-24"
ticket_url: "https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-466"
project: "hesd"
updated: "2025-11-15T07:16"
last_synced: "2025-11-15T07:16"
  - jira
  - ticket
  - closed
  - hesd
  - watchlist---

# HESD-466: ARD795 - [AppStuck] Completeness Review: Processor receiving error message when moving application to CAR status Error Message "Error occurred during processing"
## Overview
- **Status**: Closed
- **Created**: 2024-07-24
- **Project**: hesd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-466)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*