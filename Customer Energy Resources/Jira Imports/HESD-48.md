---
ticket_id: HESD-48
title: HE Internal User is unable to proceed with making internal changes to the application.
status: Resolved
created: '2022-04-12'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-48
project: hesd
tags:
- jira
- ticket
- hesd
- resolved
updated: 2025-11-15T07:17
last_synced: 2025-11-15T07:17
---

# HESD-48: HE Internal User is unable to proceed with making internal changes to the application.
## Overview
- **Status**: Resolved
- **Created**: 2022-04-12
- **Project**: hesd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-48)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*