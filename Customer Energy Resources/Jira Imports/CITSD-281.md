---
ticket_id: CITSD-281
title: ALM 72 TEST UAT - AMEND COMPONENTS - (ADD) TOTAL PANEL RATING DOES NOT AUTO
  POPULATE
status: Resolved
created: '2023-04-17'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-281
project: citsd
tags:
- jira
- ticket
- citsd
- resolved
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# CITSD-281: ALM 72 TEST UAT - AMEND COMPONENTS - (ADD) TOTAL PANEL RATING DOES NOT AUTO POPULATE
## Overview
- **Status**: Resolved
- **Created**: 2023-04-17
- **Project**: citsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-281)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*