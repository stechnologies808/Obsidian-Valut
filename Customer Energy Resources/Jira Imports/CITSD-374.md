---
ticket_id: CITSD-374
title: ARD086 - AC Disconnect spec sheet required, when ACD is selected from dropdown
  list // PV panel spec sheet not required when PV Panel not on list selected
status: Resolved
created: '2023-06-29'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-374
project: citsd
tags:
- jira
- ticket
- citsd
- resolved
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# CITSD-374: ARD086 - AC Disconnect spec sheet required, when ACD is selected from dropdown list // PV panel spec sheet not required when PV Panel not on list selected
## Overview
- **Status**: Resolved
- **Created**: 2023-06-29
- **Project**: citsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-374)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*