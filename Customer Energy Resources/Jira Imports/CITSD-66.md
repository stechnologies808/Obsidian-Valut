---
ticket_id: CITSD-66
title: 'Project Summary Page Total kW value incorrect: Does not match override Total
  Amendment Change kW field from Completeness Review.'
status: Resolved
created: '2022-03-04'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-66
project: citsd
tags:
- jira
- ticket
- citsd
- resolved
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# CITSD-66: Project Summary Page Total kW value incorrect: Does not match override Total Amendment Change kW field from Completeness Review.
## Overview
- **Status**: Resolved
- **Created**: 2022-03-04
- **Project**: citsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-66)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*