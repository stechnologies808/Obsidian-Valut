---
# Dataview Properties
ticket_id: "HESD-345"
title: "ARD610 - Customers/Contractors are unable to move past the "Start New Electrical Service Application" button."
status: "Resolved"
created: "2024-04-08"
ticket_url: "https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-345"
project: "hesd"
  - jira
  - ticket
  - hesd
  - resolved
updated: "2025-11-15T07:16"
last_synced: "2025-11-15T07:16"---

# HESD-345: ARD610 - Customers/Contractors are unable to move past the "Start New Electrical Service Application" button.
## Overview
- **Status**: Resolved
- **Created**: 2024-04-08
- **Project**: hesd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-345)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*