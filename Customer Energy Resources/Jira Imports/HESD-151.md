---
# Dataview Properties
ticket_id: "HESD-151"
title: "Multiple users reporting that they cannot view documents - "page is unavailable" [Critical - HE is unable to process/approve applications]"
status: "Resolved"
created: "2022-10-07"
ticket_url: "https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-151"
project: "hesd"
  - jira
  - ticket
  - hesd
  - resolved
updated: "2025-11-15T07:16"
last_synced: "2025-11-15T07:16"---

# HESD-151: Multiple users reporting that they cannot view documents - "page is unavailable" [Critical - HE is unable to process/approve applications]
## Overview
- **Status**: Resolved
- **Created**: 2022-10-07
- **Project**: hesd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-151)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*