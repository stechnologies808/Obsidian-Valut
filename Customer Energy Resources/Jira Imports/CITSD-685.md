---
# Dataview Properties
ticket_id: "CITSD-685"
title: "ARD488 - Validation Submittal - System Components incorrectly label different panels as "PV Panels #1" even though there are 7"
status: "Closed"
created: "2024-01-31"
ticket_url: "https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-685"
project: "citsd"
  - jira
  - ticket
  - citsd
  - closed
updated: "2025-11-15T07:16"
last_synced: "2025-11-15T07:16"---

# CITSD-685: ARD488 - Validation Submittal - System Components incorrectly label different panels as "PV Panels #1" even though there are 7
## Overview
- **Status**: Closed
- **Created**: 2024-01-31
- **Project**: citsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-685)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*