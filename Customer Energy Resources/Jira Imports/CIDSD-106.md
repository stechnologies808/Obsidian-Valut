---
# Dataview Properties
ticket_id: "CIDSD-106"
title: "Make "Loading Type" and "Metering Code" fields on 6A L6/L4 Creations page optional"
status: "Resolved"
created: "2022-02-28"
ticket_url: "https://gridunity.atlassian.net/servicedesk/customer/portal/11/CIDSD-106"
project: "cidsd"
  - jira
  - ticket
  - cidsd
  - resolved
updated: "2025-11-15T07:16"
last_synced: "2025-11-15T07:16"---

# CIDSD-106: Make "Loading Type" and "Metering Code" fields on 6A L6/L4 Creations page optional
## Overview
- **Status**: Resolved
- **Created**: 2022-02-28
- **Project**: cidsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/11/CIDSD-106)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*