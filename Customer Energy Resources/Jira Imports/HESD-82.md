---
ticket_id: HESD-82
title: 'PROD: Contractor unable to make correction when project is in RC/CAR.'
status: Resolved
created: '2022-05-19'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-82
project: hesd
tags:
- jira
- ticket
- hesd
- resolved
updated: 2025-11-15T07:17
last_synced: 2025-11-15T07:17
---

# HESD-82: PROD: Contractor unable to make correction when project is in RC/CAR.
## Overview
- **Status**: Resolved
- **Created**: 2022-05-19
- **Project**: hesd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-82)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*