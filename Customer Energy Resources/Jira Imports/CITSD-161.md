---
ticket_id: CITSD-161
title: Validation fail email not sent to customer/contractor after validation fail.
  Unclear if event history correctly records the rejection.
status: Resolved
created: '2022-12-28'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-161
project: citsd
tags:
- jira
- ticket
- citsd
- resolved
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# CITSD-161: Validation fail email not sent to customer/contractor after validation fail. Unclear if event history correctly records the rejection.
## Overview
- **Status**: Resolved
- **Created**: 2022-12-28
- **Project**: citsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-161)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*