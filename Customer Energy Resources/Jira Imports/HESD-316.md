---
# Dataview Properties
ticket_id: "HESD-316"
title: "The "Revision Accepted" (revision passed) email in CIT Prod is being sent from "noreply@gridunity.com" and is in plain text instead of html."
status: "Closed"
created: "2024-02-26"
ticket_url: "https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-316"
project: "hesd"
  - jira
  - ticket
  - closed
  - hesd
updated: "2025-11-15T07:16"
last_synced: "2025-11-15T07:16"---

# HESD-316: The "Revision Accepted" (revision passed) email in CIT Prod is being sent from "noreply@gridunity.com" and is in plain text instead of html.
## Overview
- **Status**: Closed
- **Created**: 2024-02-26
- **Project**: hesd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-316)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*