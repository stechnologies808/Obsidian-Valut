---
ticket_id: CIDSD-23
title: 'Reporting: Some SAP data fields not available in the csv export'
status: Resolved
created: '2021-11-15'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/11/CIDSD-23
project: cidsd
tags:
- jira
- ticket
- cidsd
- resolved
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# CIDSD-23: Reporting: Some SAP data fields not available in the csv export
## Overview
- **Status**: Resolved
- **Created**: 2021-11-15
- **Project**: cidsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/11/CIDSD-23)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*