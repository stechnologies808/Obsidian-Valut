---
ticket_id: HESD-96
title: Please update DEV email template to reflect correction made in PROD, STAGE
  and TEST
status: Resolved
created: '2022-06-13'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-96
project: hesd
tags:
- jira
- ticket
- hesd
- resolved
updated: 2025-11-15T07:17
last_synced: 2025-11-15T07:17
---

# HESD-96: Please update DEV email template to reflect correction made in PROD, STAGE and TEST
## Overview
- **Status**: Resolved
- **Created**: 2022-06-13
- **Project**: hesd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-96)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*