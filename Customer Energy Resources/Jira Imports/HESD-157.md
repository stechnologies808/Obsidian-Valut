---
# Dataview Properties
ticket_id: "HESD-157"
title: "480542: Failed CR status showing "initiated", expected to be CAR"
status: "Closed"
created: "2022-10-31"
ticket_url: "https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-157"
project: "hesd"
  - jira
  - ticket
  - closed
  - hesd
updated: "2025-11-15T07:16"
last_synced: "2025-11-15T07:16"---

# HESD-157: 480542: Failed CR status showing "initiated", expected to be CAR
## Overview
- **Status**: Closed
- **Created**: 2022-10-31
- **Project**: hesd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-157)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*