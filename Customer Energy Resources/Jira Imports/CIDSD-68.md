---
ticket_id: CIDSD-68
title: Stage - Defect 288 -  Automatic re-login after hitting LOGOUT button
status: Resolved
created: '2022-01-11'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/11/CIDSD-68
project: cidsd
tags:
- jira
- ticket
- cidsd
- resolved
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# CIDSD-68: Stage - Defect 288 - Automatic re-login after hitting LOGOUT button
## Overview
- **Status**: Resolved
- **Created**: 2022-01-11
- **Project**: cidsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/11/CIDSD-68)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*