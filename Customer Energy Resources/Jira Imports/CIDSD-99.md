---
ticket_id: CIDSD-99
title: Disable Send Email functionality in the User Action dropdown
status: Resolved
created: '2022-02-22'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/11/CIDSD-99
project: cidsd
tags:
- jira
- ticket
- cidsd
- resolved
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# CIDSD-99: Disable Send Email functionality in the User Action dropdown
## Overview
- **Status**: Resolved
- **Created**: 2022-02-22
- **Project**: cidsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/11/CIDSD-99)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*