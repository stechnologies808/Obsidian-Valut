---
ticket_id: HESD-169
title: Contractor unable to start application. External list is missing the internal
  ID for some of the companies
status: Closed
created: '2022-12-06'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-169
project: hesd
tags:
- jira
- ticket
- closed
- hesd
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# HESD-169: Contractor unable to start application. External list is missing the internal ID for some of the companies
## Overview
- **Status**: Closed
- **Created**: 2022-12-06
- **Project**: hesd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-169)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*