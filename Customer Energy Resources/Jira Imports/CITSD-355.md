---
ticket_id: CITSD-355
title: ARD067 - Could the calendar popup for date fields indicate today's date? Prod
  has it select today's date by default when the popup is brought  up.
status: Resolved
created: '2023-06-23'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-355
project: citsd
tags:
- jira
- ticket
- citsd
- resolved
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# CITSD-355: ARD067 - Could the calendar popup for date fields indicate today's date? Prod has it select today's date by default when the popup is brought up.
## Overview
- **Status**: Resolved
- **Created**: 2023-06-23
- **Project**: citsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-355)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*