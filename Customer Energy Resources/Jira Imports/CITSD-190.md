---
ticket_id: CITSD-190
title: ALM-27 ICLM UAT Role HEEngineer unable to click activity link; redirects to
  Landing page
status: Resolved
created: '2023-03-17'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-190
project: citsd
tags:
- jira
- ticket
- citsd
- resolved
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# CITSD-190: ALM-27 ICLM UAT Role HEEngineer unable to click activity link; redirects to Landing page
## Overview
- **Status**: Resolved
- **Created**: 2023-03-17
- **Project**: citsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-190)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*