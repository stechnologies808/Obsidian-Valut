---
ticket_id: CITSD-275
title: ALM 98 TEST - ITR workflow > Downtown network screen and Transient Overvoltage
  screens not recalculating based on user input
status: Resolved
created: '2023-04-15'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-275
project: citsd
tags:
- jira
- ticket
- citsd
- resolved
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# CITSD-275: ALM 98 TEST - ITR workflow > Downtown network screen and Transient Overvoltage screens not recalculating based on user input
## Overview
- **Status**: Resolved
- **Created**: 2023-04-15
- **Project**: citsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-275)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*