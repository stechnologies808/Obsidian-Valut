---
ticket_id: CIDSD-93
title: Route the user to the Project Summary page after 6A L4/L6 creation
status: Resolved
created: '2022-02-10'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/11/CIDSD-93
project: cidsd
tags:
- jira
- ticket
- cidsd
- resolved
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# CIDSD-93: Route the user to the Project Summary page after 6A L4/L6 creation
## Overview
- **Status**: Resolved
- **Created**: 2022-02-10
- **Project**: cidsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/11/CIDSD-93)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*