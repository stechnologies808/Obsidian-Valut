---
ticket_id: CITSD-110
title: Liability Insurance fields appear during Validation despite kW threshold not
  being met
status: Resolved
created: '2022-10-20'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-110
project: citsd
tags:
- jira
- ticket
- citsd
- resolved
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# CITSD-110: Liability Insurance fields appear during Validation despite kW threshold not being met
## Overview
- **Status**: Resolved
- **Created**: 2022-10-20
- **Project**: citsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-110)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*