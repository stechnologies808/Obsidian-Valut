---
ticket_id: CIDSD-101
title: Unable to update 'i-info' on the Stakeholder name field. Is this b/c its a
  field table?
status: Resolved
created: 2022-02-23
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/11/CIDSD-101
project: cidsd
tags:
- jira
- ticket
- cidsd
- resolved
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# CIDSD-101: Unable to update 'i-info' on the Stakeholder name field. Is this b/c its a field table?
## Overview
- **Status**: Resolved
- **Created**: 2022-02-23
- **Project**: cidsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/11/CIDSD-101)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*