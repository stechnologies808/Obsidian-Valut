---
ticket_id: HESD-102
title: CID Contractor unable to start new electrical request; redirects to landing
  page
status: Resolved
created: '2022-06-16'
ticket_url: https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-102
project: hesd
tags:
- jira
- ticket
- hesd
- resolved
updated: 2025-11-15T07:16
last_synced: 2025-11-15T07:16
---

# HESD-102: CID Contractor unable to start new electrical request; redirects to landing page
## Overview
- **Status**: Resolved
- **Created**: 2022-06-16
- **Project**: hesd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-102)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*