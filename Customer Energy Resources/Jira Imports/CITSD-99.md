---
# Dataview Properties
ticket_id: "CITSD-99"
title: "REGRESSION TESTING - Email templates where "Application" is spelled incorrectly and program abbreviation is used"
status: "Resolved"
created: "2022-03-23"
ticket_url: "https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-99"
project: "citsd"
  - jira
  - ticket
  - citsd
  - resolved
updated: "2025-11-15T07:16"
last_synced: "2025-11-15T07:16"---

# CITSD-99: REGRESSION TESTING - Email templates where "Application" is spelled incorrectly and program abbreviation is used
## Overview
- **Status**: Resolved
- **Created**: 2022-03-23
- **Project**: citsd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/19/CITSD-99)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*