---
# Dataview Properties
ticket_id: "HESD-2"
title: "0316 SSO Link redirecting to "It appears as though you were sent to a page that does not exist""
status: "Closed"
created: "2022-03-16"
ticket_url: "https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-2"
project: "hesd"
  - jira
  - ticket
  - closed
  - hesd
updated: "2025-11-15T07:16"
last_synced: "2025-11-15T07:16"---

# HESD-2: 0316 SSO Link redirecting to "It appears as though you were sent to a page that does not exist"
## Overview
- **Status**: Closed
- **Created**: 2022-03-16
- **Project**: hesd
- **Request Type**: 
## People
- **Assignee**: 
- **Reporter**: 
## Business Context
- **Business Impact**: Not specified
- **Business Value**: Not specified
- **HE Project**: Not specified
## Description
No description available
## Comments
No comments available.
## Links
- [View in Jira](https://gridunity.atlassian.net/servicedesk/customer/portal/20/HESD-2)
- Original Summary: 
## Metadata
- **Scraped**: Unknown
- **Source**: JSON export from Jira Service Management
---
*This file was automatically generated from JSON data*